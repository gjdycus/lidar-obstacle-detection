...
void proximity(const std::vector<std::vector<float>> points, int i, std::vector<int> &cluster, KdTree* tree, float distanceTol, std::vector<bool> &processedPoints)
{
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	while (i < points.size())
	{
		...
		proximity(points, i, cluster, tree, distanceTol, processedPoints);
...
int main ()
{
	...
	// std::vector<int> nearby = tree->search({7.5,6},3.0);
